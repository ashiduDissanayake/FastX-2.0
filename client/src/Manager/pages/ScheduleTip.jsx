...
const ScheduleTrip = () => {
    ...
    const loadOrdersByStore = async (storeId) => {
        try {
            const response = await axios.get(`http://localhost:8080/manager/getstoreorders/${storeId}`);
    ...
    const handleOrderSelection = (orderId, capacity) => {
        setSelectedOrders((prevOrders) => {
            ...
            if (!isSelected && newTotalCapacity > 500) {
                setWarning(`Cannot select order! Selecting this order will exceed the capacity limit of 500. Current capacity: ${totalCapacity}`);
                ...
                {selectedStore && (
                    <div className="mb-4">
                            ...
                            {orderData.length > 0 ? (
                                orderData.map((order) => (
                                    <div key={order.order_ID} className="flex items-center">
                                        ...
                                        />
                                        <span>
                                            Order #{order.order_ID}, Route #{order.route_ID}, Capacity: {order.capacity}
                                        </span>
                                    </div>
                                ))
                            ) : (
                        ...
                        <div className="mt-4">
                            ...
                            <p className="text-sm mt-1">{totalCapacity} / 500 Capacity Used</p>
                        </div>
                    </div>
                )}
                            ...
                            >
                                ...
                                {assistantData.map((assistant) => (
                                    <option key={assistant.assistant_ID} value={assistant.assistant_ID} disabled={assistant.status !== "inactive" || assistant.current_working_time === 60}>
                                        {`${assistant.assistant_ID} - worked ${assistant.current_working_time} hrs`} {assistant.status === "inactive" ? "" : (assistant.status === "active1" ? "(active1)" : "(active2)")}
                                    </option>
                                ))}
                            </select>
                        </div>
