...
const Shop = () => {
  ...
  const getRandomSize = () => {
    const sizes = [
      { height: "h-48", width: "w-full", span: "col-span-1" },
      { height: "h-64", width: "w-full", span: "col-span-1" },
      { height: "h-96", width: "w-full", span: "col-span-2" },
      { height: "h-80", width: "w-full", span: "col-span-1" },
    ];
  ...
  useEffect(() => {
    ...
    setPage(1);
    setHasMore(true);
  }, [activeTab, selectedCategory, selectedSubCategory, sortBy, refreshKey]);
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/user/${
          activeTab === "new" ? "new_arrivals" : "trending"
        }?page=${page}&limit=20&category=${selectedCategory}&subcategory=${selectedSubCategory}&sortBy=${sortBy}`,
        {
          credentials: "include",
          headers: {
            "Content-Type": "application/json",
      ...
      const hasMoreProducts =
        data.hasMore !== undefined ? data.hasMore : newProducts.length === 20;

      setProducts((prev) => {
        const combinedProducts = [...prev, ...newProducts];
        return Array.from(
          new Map(
            combinedProducts.map((item) => [item.product_ID, item])
          ).values()
        );
      });
    ...
    <div className="relative">
      ...
      >
        <span className="text-pink-100">
          {value === "all" ? placeholder : value || placeholder}
        </span>
        <ChevronDown
        ...
        {selectedCategory !== "all" && (
          <div>
            ...
            <Dropdown
              ...
              options={{
                ...categories[selectedCategory].reduce(
                  (acc, curr) => ({ ...acc, [curr.toLowerCase()]: [curr] }),
                  {}
                ),
              }}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center space-x-4">
            ...
            >
              <RefreshCw className="w-5 h-5 text-pink-300" />
              <span>Refresh</span>
            </motion.button>
            <motion.button
              whileHover={{ scale: 1.05 }}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              <AnimatePresence mode="popLayout">
                {products.map((product, index) => (
                  <ProductCard
                    key={`${product.product_ID}-${refreshKey}`}
                    product={product}
