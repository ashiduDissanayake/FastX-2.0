...
function Store5() {
  ...
  const fetchOrders = () => {
    fetch('http://localhost:8080/mainmanager/Store5/orders', {
      method: 'GET',
  ...
  const fetchTrainCapacity = () => {
    fetch('http://localhost:8080/mainmanager/train/nearest-capacity/5', {
      credentials: 'include'
    })
  ...
  const handleStatusChange = (orderId, orderCapacity) => {
    fetch(`http://localhost:8080/mainmanager/orders/${orderId}`, {
      ...
      .then(() => {
        return fetch('http://localhost:8080/mainmanager/train/reduce-capacity/5', {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
