...
const driverController = {
  ...
  // Check Auth
  checkAuth: (req, res) => {
    const token = req.cookies.driver; // Get the token from cookies
    if (!token) return res.json({ isAuthenticated: false });

    jwt.verify(token, process.env.SECRET2, (err, user) => {
    ...
    DriverModel.getAllSchedules((error, results) => {
      if (error) {
        res
          .status(500)
          .json({
            error: "An error occurred while fetching the schedule data",
          });
      } else {
