...
function Store1() {
  ...
  const fetchOrders = () => {
    fetch('http://localhost:8080/mainmanager/Store6/orders', {
      method: 'GET',
  ...
  const fetchTrainCapacity = () => {
    fetch('http://localhost:8080/mainmanager/train/nearest-capacity/6', {
      credentials: 'include'
    })
  ...
  const handleStatusChange = (orderId, orderCapacity) => {
    fetch(`http://localhost:8080/mainmanager/orders/${orderId}`, {
      ...
      .then(() => {
        return fetch('http://localhost:8080/mainmanager/train/reduce-capacity/6', {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
